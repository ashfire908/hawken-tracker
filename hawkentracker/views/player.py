...
def view(target):
    ...
    # Access check
    if not permissions_view.player.player(player.player_id).view:
        if player.opt_out:
    ...
    context = {
        ...
        "edit": {
            "settings": permissions_view.player.player(player.player_id).settings,
            "blacklist": permissions_view.player.blacklist.change
        },
        "ranking": False,
        "stats": False,
        "mechs": False,
        "matches": permissions_view.player.player(player.player_id).match.list,
        "groups": False
    }
    ...
    # Rankings
    if permissions_view.player.player(player.player_id).rankings:
        if player.stats is None:
            context["ranking"] = None
        else:
            # Ranked stats
            show_stats = permissions_view.player.player(player.player_id).stats.ranked
    ...
    # Global stats
    if permissions_view.player.player(player.player_id).stats.overall:
        context["stats"] = {}

    # Mech stats
    if permissions_view.player.player(player.player_id).stats.mech:
        context["mechs"] = []

    # Groups
    if permissions_view.player.player(player.player_id).group:
        context["groups"] = []
    ...
    if context["linked"]["status"]:
        if permissions_view.player.player(player.player_id).link.user:
            context["linked"]["user"] = {
                "id": player.user.user_id,
                "name": player.user.username
            }

        if permissions_view.player.player(player.player_id).link.players:
            players = Player.query.filter(Player.link_user == player.user.user_id).filter(Player.link_status == LinkStatus.linked).filter(Player.player_id != guid)
            context["linked"]["players"] = [player.callsign or player.player_id for player in players]
...
def settings(target):
    ...
    # Access check
    if not permissions_view.player.player(player.player_id).settings:
